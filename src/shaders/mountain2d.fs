...
float noise( in vec3 x )
{
	...
	vec2 uv2 = uv1  + offz;
	return mix(texture2D( noiseTex, uv1, -1000.0).x,texture2D( noiseTex, uv2, -1000.0).x,fract(z))-0.5;
}
...
void main()	{
    ...
    vec3 campos = vec3(80.0,400.0,_time*3.0);
    vec3 ray   = rotate(normalize(vec3(uv.x,uv.y-sin(_time*0.02)*0.2-0.2,1.0).xyz),_time*0.01);
